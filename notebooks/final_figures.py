...
all_data = pd.read_csv("/Users/sebs_mac/uni_OneDrive/honours/data/optimised_model_metrics/reconstruction_validation/k_fold_validation_marginal.csv")


datasets = ["gb1", "a4", "gcn4", "gfp", "mafg"]
for protein in datasets:
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))  # Adjusted the figure size for better visibility
    ...
    lambda x: "ae" if "ae" in x.split("_") else 
              ("a" if "a" in x.split("_") else 
              ("e" if "e" in x.split("_") else 
              ("train" if "train" in x.split("_") else None)))
    )   

    category_order = ["train", "ae", "a", "e"]
    x_tick_titles = ["Ancestor/Extant train", "Ancestor/Extant test", "Ancestor 5-fold val", "Extant 5-fold val"]
    
    # # Create the barplot with seaborn
    bar = sns.barplot(x="category", y="marginal", data=subset, hue=subset["category"], palette=sns.color_palette("Set2"), ax=ax, order=category_order, dodge=False)
    ...
    new_labels = ["Ancestors", "Ancestors/Extants test", "Extants", "Ancestors/Extants train"]
...
plt.title("Validation MSA pariwise covariance correlation with reconstruction covariance (Validation models)")
plt.show() 
...
data.to_csv("/Users/sebs_mac/uni_OneDrive/honours/data/overfit_testing/overfit_test_results.csv")

# +
data = pd.read_csv("/Users/sebs_mac/uni_OneDrive/honours/data/overfit_testing/overfit_test_results.csv")

datasets = ["gb1", "a4", "gcn4", "gfp", "mafg"]
metrics = ["Training_ELBO", "Ancestor_ELBO", "Extant_ELBO"]
# Melt the DataFrame to long format
melted_data = pd.melt(data, id_vars=['family'], value_vars=metrics, var_name='metric', value_name='value')
...
g.set_axis_labels("Protein Family")
g.set_ylabels("ELBO")
plt.title("Final ELBO on holdouts of extants or ancestors after training on Ancestors & extants")
g.add_legend()
plt.show()      
...
all_data = pd.read_csv("/Users/sebs_mac/uni_OneDrive/honours/data/optimised_model_metrics/zero_shot_tasks/zero_shot_optimised_metrics.csv")
datasets = ["gb1", "a4", "gcn4", "gfp", "mafg"]
metrics = ["spearman_rho", "top_k_recall", "ndcg", "roc_auc"]
# Melt the DataFrame to long format
melted_data = pd.melt(all_data, id_vars=['family', 'category'], value_vars=metrics, var_name='metric', value_name='value')
sns.set_style("darkgrid")  
...
all_data = pd.read_csv("/Users/sebs_mac/uni_OneDrive/honours/data/optimised_model_metrics/curated/curated_metrics.csv")
melted_data = pd.melt(all_data, id_vars=['family', 'category'], value_vars=metrics, var_name='metric', value_name='value')
sns.set_style("darkgrid")  
metrics = ["spearman_rho", "top_k_recall", "ndcg", "roc_auc"]
for metric in metrics:
...
# +
all_data = pd.read_csv("/Users/sebs_mac/uni_OneDrive/honours/data/optimised_model_metrics/clustering/clustering_metrics.csv")
datasets = ["gb1", "a4", "gcn4", "gfp", "mafg"]
...
for metric in metrics:
    ...
    for protein, size in zip(datasets, train_size):
        ...
        sns.lineplot(x="extant", y=metric, data=subset, ax=ax, label=f"{protein.upper()} ({size} seqs)")
...
covariances = pd.read_csv("/Users/sebs_mac/uni_OneDrive/honours/data/optimised_model_metrics/zero_shot_tasks/pairwise_covariance.csv")
covariances
g = sns.catplot(
data=covariances,
x="family",
...
g.set_ylabels("Pearson correlation coefficient (r)")
g.add_legend(title="Training data")
plt.title("Extant MSA pariwise covariance correlation with reconstruction covariance (zero shot models)")
plt.show() 
# -
...
covariances = pd.read_csv("/Users/sebs_mac/uni_OneDrive/honours/data/optimised_model_metrics/curated/curated_extant_covariances.csv")
covariances
g = sns.catplot(
data=covariances,
x="family",
...
g.set_ylabels("Pearson correlation coefficient (r)")
g.add_legend(title="Training data")
plt.title("Extant MSA pariwise covariance correlation with reconstruction covariance (Curated models)")
plt.show() 
# -
...
vars = ["/Users/sebs_mac/uni_OneDrive/honours/data/optimised_model_metrics/clustering/clustering_extant_covariances.csv", 
...
for v, t in zip(vars, titles):
    ...
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))  # Create a single figure for all lines

    # Iterate through each dataset and plot them on the same axes
    for protein, size in zip(datasets, train_size):
        subset = all_data[all_data["family"] == protein.upper()]
        subset = subset.sort_values(by="extant_proportion")
        sns.scatterplot(x="extant_proportion", y="pearson", data=subset, ax=ax, marker="o")
        
        sns.lineplot(x="extant_proportion", y="pearson", data=subset, ax=ax, label=f"{protein.upper()} ({size} seqs)")

    # Set the title and legend
    plt.title(f"Pearson correlation of pairwise covariances between {t} and reconstructions")
...
vars = ["/Users/sebs_mac/uni_OneDrive/honours/data/optimised_model_metrics/clustering/clustering_extant_hamming.csv", 
...
for v, t in zip(vars, titles):
    ...
    for protein in datasets:
        subset = all_data[all_data["family"] == protein.upper()]
        subset = subset.sort_values(by="extant_proportion")
        sns.scatterplot(x="extant_proportion", y="hamming_distance", data=subset, ax=ax, marker="o")
        
        sns.lineplot(x="extant_proportion", y="hamming_distance", data=subset, ax=ax, label=protein.upper())
...
wts = ["SPG1_STRSG/1-448", "A4_HUMAN/1-770", "GCN4_YEAST/1-281", "GFP_AEQVI/1-238", "MAFG_MOUSE/1-41"]

with open("../data/dummy_config.yaml", "r") as stream:
    settings = yaml.safe_load(stream)

sns.set_style("darkgrid")  
for p, id, wt, in zip(protein, tree_id, wts):
    for d, lab in zip(data, labels):
        state_dict = f"/Users/sebs_mac/uni_OneDrive/honours/data/vis_models/{p}_{d}_r1_model_state.pt"
        tree_seqs = f"/Users/sebs_mac/uni_OneDrive/honours/data/vis_models/{p}_tree_{id}.aln"

        vs.vis_tree(wt, tree_seqs, state_dict, settings, f"{p.upper()} - {lab} model", rgb=True)
        vs.vis_tree(wt, tree_seqs, state_dict, settings,  f"{p.upper()} - {lab} model", rgb=True, lower_2d=True)
...
data = ["ae", "a", "e"]
labels = ["Ancestor/Extant", "Ancestor", "Extant"]
wts =  ["A4_HUMAN/1-770", "GCN4_YEAST/1-281", "GFP_AEQVI/1-238", "MAFG_MOUSE/1-41", "SPG1_STRSG/1-448"]
...
sns.set_style("darkgrid")  
for p, wt, v, frac, id, dims in zip(protein, wts, variants, fracs, tree_id, latent_dims):
    for d, lab, in zip(data, labels):
        ...
        tree_seqs = f"/Users/sebs_mac/uni_OneDrive/honours/data/vis_models/{p}_tree_{id}.aln"
        vs.visualise_variants(settings, v, state_dict, f"{p.upper()} - {lab} model", tree_seqs, wt, vis_2D=True, frac=frac)
...
for p, wt, v, frac, id in zip(protein, wts, variants, fracs, tree_id):
    for d, lab, in zip(data, labels):
        ...
        tree_seqs = f"/Users/sebs_mac/uni_OneDrive/honours/data/vis_models/{p}_tree_{id}.aln"
        state_dict = state_dict = f"/Users/sebs_mac/uni_OneDrive/honours/data/vis_models/{p}_{d}_r1_model_state.pt"
        test = vs.visualise_variants(settings, v, state_dict, f"{p.upper()} - {lab} model", tree_seqs, wt, vis_2D=False, frac=frac)
...
data = pd.read_csv("/Users/sebs_mac/uni_OneDrive/honours/data/optimised_model_metrics/zero_shot_tasks/top_models/all_top_model.csv")
...
for prot, mutations, anc, ext in zip(proteins, mutation_positions, ancestor_counts, extant_counts):
...
bar = sns.barplot(x="experiment", )

# +
data = pd.read_csv("/Users/sebs_mac/uni_OneDrive/honours/data/overfit_testing/all_embedding_closest_results.csv")
data = data[["experiment", "Ancestor_to_Ancestor", "Ancestor_to_Extant", "Extant_to_Extant"]]
datasets = ["gb1", "a4", "gcn4", "gfp", "mafg"]

metrics = ["Ancestor_to_Ancestor", "Ancestor_to_Extant", "Extant_to_Extant"]
# Melt the DataFrame to long format
melted_data = pd.melt(data, id_vars=['experiment'], value_vars=metrics, var_name='metric', value_name='value')
melted_data
sns.set_style("darkgrid")  
...
plt.title("Proportion of closest sequence pairs using ESM2 embeddings and cosine similarity")
g.add_legend()
plt.show()      

...
xs_train = np.array(
    [st.seq_to_one_hot(x).flatten() for x in train_data["mutated_sequence"]]
)
ys_train = np.array([float(y) for y in train_data["DMS_score"]])
xs_test = np.array(
    [st.seq_to_one_hot(x).flatten() for x in test_data["mutated_sequence"]]
)
...
results, grids = perform_grid_search(
    mafg_oh_xs_train, mafg_oh_ys_train, cls_list, param_grid_list, pipe
)
spear = {}
for grid in grids:
...
def get_transformer_embeddings(
    model, alphabet, batch_converter, data, device, model_layer=-1
) -> pd.DataFrame:
    """Get mean representation of sequence"""

    id_seq_pair = [
        (id, seq) for id, seq in zip(data["mutant"], data["mutated_sequence"])
    ]
    ...
    embeddings = pd.DataFrame(
        {"mutant": batch_labels, "embedding": sequence_representations}
    )
...
def one_hot_top_model(train_data, test_data):
    ...
    for grid in grids:
        print(
            grid.best_estimator_.get_params()["steps"][0][1]
...
train_mafg_esm = get_transformer_embeddings(
    model, alphabet, batch_converter, mafg_train, model_layer=6, device=device
)
mafg_train = mafg_train.merge(train_mafg_esm, on="mutant")
test_mafg_esm = get_transformer_embeddings(
    model, alphabet, batch_converter, mafg_test, model_layer=6, device=device
)
...
results, grids = perform_grid_search(
    mafg_Xs_train, mafg_ys_train, cls_list, param_grid_list, pipe
)
for grid in grids:
    print(
        grid.best_estimator_.get_params()["steps"][0][1]
...
mafg_oh_xs_train = np.array(
    [st.seq_to_one_hot(x).flatten() for x in mafg_train["mutated_sequence"]]
)
print(mafg_oh_xs_train.shape)
mafg_oh_ys_train = np.array([float(y) for y in mafg_train["DMS_score"]])
print(mafg_oh_ys_train.shape)

mafg_oh_xs_test = np.array(
    [st.seq_to_one_hot(x).flatten() for x in mafg_test["mutated_sequence"]]
)
...
def train_and_fit_vae_top_model(
    train_data,
    test_data,
    state,
    label,
...
for p, v, dims in zip(protein, datasets, latent_dims):
    for (
        ...
        cls_list, param_grid_list, pipe = instantiate_top_models_and_pipe()
        results, grids = train_and_fit_vae_top_model(
            train, test, state_dict, lab, p, cls_list, param_grid_list, pipe
        )
...
states = [
    data_path + f"mafg_{prop}/mafg_{prop}_r1/mafg_{prop}_r1_model_state.pt"
    for prop in labels
]
results, grids = train_and_fit_vae_top_model(
    mafg_train,
...
train_gcn4_esm = get_transformer_embeddings(
    gnc4_model, alphabet, batch_converter, gcn4_train, model_layer=6, device=device
)
gcn4_train = gcn4_train.merge(train_gcn4_esm, on="mutant")

test_gcn4_esm = get_transformer_embeddings(
    gnc4_model, alphabet, batch_converter, gcn4_test, model_layer=6, device=device
)
...
results, grids = perform_grid_search(
    gcn4_Xs_train, gcn4_ys_train, cls_list, param_grid_list, pipe
)
for grid in grids:
    print(
        grid.best_estimator_.get_params()["steps"][0][1]
...
# +
gcn4_oh_xs_train = np.array(
    [st.seq_to_one_hot(x).flatten() for x in gcn4_train["mutated_sequence"]]
)
gcn4_oh_ys_train = np.array([float(y) for y in gcn4_train["DMS_score"]])

gcn4_oh_xs_test = np.array(
    [st.seq_to_one_hot(x).flatten() for x in gcn4_test["mutated_sequence"]]
)
...
results, grids = perform_grid_search(
    gcn4_oh_xs_train, gcn4_oh_ys_train, cls_list, param_grid_list, pipe
)
for grid in grids:
    print(
        grid.best_estimator_.get_params()["steps"][0][1]
...
states = [
    data_path
    + f"gcn4_{prop}/gcn4_{prop}_extants_r1/gcn4_{prop}_extants_r1_model_state.pt"
    for prop in labels
]
...
model = model.to(device)
model.eval()
train_gfp_esm = get_transformer_embeddings(
    model, alphabet, batch_converter, gfp_train, model_layer=6, device=device
)
gfp_train = gfp_train.merge(train_mafg_esm, on="mutant")

test_gfp_esm = get_transformer_embeddings(
    model, alphabet, batch_converter, gfp_test, model_layer=6, device=device
)
...
results, grids = perform_grid_search(
    gfp_Xs_train, gfp_ys_train, cls_list, param_grid_list, pipe
)
for grid in grids:
    print(
        grid.best_estimator_.get_params()["steps"][0][1]
...
states = [
    data_path + f"gfp_{prop}/gfp_{prop}_extants_r1/gfp_{prop}_extants_r1_model_state.pt"
    for prop in labels
]
results, grids = train_and_fit_vae_top_model(
    gfp_train,
...
model.eval()
model = model.to(device)
train_a4_esm = get_transformer_embeddings(
    model, alphabet, batch_converter, a4_train, model_layer=6, device=device
)
a4_train = a4_train.merge(train_a4_esm, on="mutant")

test_a4_esm = get_transformer_embeddings(
    model, alphabet, batch_converter, a4_test, model_layer=6, device=device
)
...
results, grids = perform_grid_search(
    a4_Xs_train, a4_ys_train, cls_list, param_grid_list, pipe
)
for grid in grids:
    print(
        grid.best_estimator_.get_params()["steps"][0][1]
...
results, grids = train_and_fit_vae_top_model(
    a4_train, a4_test, states, labels, "A4", cls_list, param_grid_list, pipe, settings
)
# -
...
states = [
    data_path + f"a4_{prop}/a4_{prop}_extants_r1/a4_{prop}_extants_r1_model_state.pt"
    for prop in labels
]
results, grids = train_and_fit_vae_top_model(
    a4_train, a4_test, states, labels, "A4", cls_list, param_grid_list, pipe, settings
...
batch_converter = alphabet.get_batch_converter()
model.eval()
train_gb1_esm = get_transformer_embeddings(
    model, alphabet, batch_converter, gb1_train, model_layer=6
)
gb1_train = gb1_train.merge(train_gb1_esm, on="mutant")

test_gb1_esm = get_transformer_embeddings(
    model, alphabet, batch_converter, gb1_test, model_layer=6
)
...
results, grids = perform_grid_search(
    gb1_Xs_train, gb1_ys_train, cls_list, param_grid_list, pipe
)
for grid in grids:
    print(
        grid.best_estimator_.get_params()["steps"][0][1]
...
states = [
    data_path + f"gb1_{prop}/gb1_{prop}_r1/gb1_{prop}_r1_model_state.pt"
    for prop in labels
]
results, grids = train_and_fit_vae_top_model(
    gb1_train,

...
def perform_grid_search(train_data, test_data, classifier_classes: list, 
                        grid_parameters: list, pipeline: Pipeline, cv=5) -> Tuple[list, list]:
    result_list = []
    grid_list = []
...
def train_and_fit_top_model(train, test):
    ...
    xs_train = np.array([st.seq_to_one_hot(x).flatten() for x in  train["mutated_sequence"]])
    ys_train = np.array([float(y) for y in train["DMS_score"]])
    ys_train_bin = np.array([float(y) for y in train["DMS_score_bin"]])
    
    xs_test = np.array([st.seq_to_one_hot(x).flatten() for x in  test["mutated_sequence"]])
...
if __name__ == "__main__":
    ...
    protein = ["gfp"]
    datasets = ["/Users/sebs_mac/git_repos/dms_data/DMS_ProteinGym_substitutions/GFP_AEQVI_Sarkisyan_2016.csv"] 
    ...
    with open("gfp_one_hot_top_model_results.csv", "a") as f:
        ...
        for p, v in zip(protein, datasets):
            for train in training:
                ...
                for model in models:
                    for t in training:
                        f.write(f"{model},{p},{t},{results[model][f'{t}_spearmanr']},{results[model][f'{t}_k_recall']},"

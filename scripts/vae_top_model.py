...
using latent space coordinates of a VAE. 
"""
...
def get_vae_latent(model, loader, orig_data):
    ...
    latent = latent.merge(
        orig_data[["mutant", "DMS_score", "DMS_score_bin"]], on="mutant"
    )
...
if __name__ == "__main__":
    ...
    for p, v, dims in zip(protein, datasets, latent_dims):
        ...
        for (
            ...
            results, grids, spearman = train_and_fit_vae_top_model(
                train,
                test,
                state_dict,
                lab,
                ...
                for t in training:
                    final.append(
                        [
                            ...
                            lab,
                            t,
                            spearman[model][f"{t}_spearmanr"],
                            spearman[model][f"{t}_k_recall"],
                            spearman[model][f"{t}_ndcg"],
                            spearman[model][f"{t}_roc_auc"],
                        ]
                    )

...
if __name__ == "__main__":
    ...
    with open("esm_top_model_results.csv", "a") as f:
        f.write("model,family,train_set,spearman,k_recall,ndcg,roc_auc\n")
        for p, v, e in zip(protein, datasets, ems_paths):

            embeddings = pd.read_pickle(e)
            variants = pd.read_csv(v)
            variants.rename(columns={"mutant": "id"}, inplace=True)
            variants = embeddings.merge(variants, on="id", how="inner")   
